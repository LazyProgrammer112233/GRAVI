...
import {
    ArrowLeft, Layers, Star, ShieldCheck, CheckCircle,
    AlertTriangle, Eye, Package, BarChart3, Sparkles,
    Cpu, ScanLine, MessageSquare, TrendingUp, Activity
} from 'lucide-react';
...
function AnimatedRing({ score, size = 140, label = 'Score' }) {
    const r = size * 0.41;
...
// ── MAIN DASHBOARD ────────────────────────────────────────────────────────────
export default function AnalysisDashboardV2() {
    const { id } = useParams();
    ...
    useEffect(() => {
        const raw = localStorage.getItem(`gravi_v2_3layer_analysis_${id}`);
    ...
    return (
        <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', paddingBottom: '3rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ fontWeight: 700, color: '#f8fafc', fontSize: '1rem' }}>Deep Vision Analysis (v2)</span>
                    </div>
                    <div style={{ display: 'flex', gap: 6, marginTop: 6, flexWrap: 'wrap' }}>
                        <LayerBadge layer={1} name="YOLOv8 Detection" color="#f97316" />
                        <LayerBadge layer={2} name="Qwen2.5-VL Reasoning" color="#a78bfa" />
                        <LayerBadge layer={3} name="OCR Extraction" color="#38bdf8" />
                    </div>
                </div>
            ...
            {/* ROW 1: Identity + Authenticity Score */}
            <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: '1.5rem' }}>
                <Panel>
                    <PanelHeader icon={Activity} title="Store Identity" iconColor="#3b82f6" />
                    <div style={{ fontSize: '1.7rem', fontWeight: 800, color: '#f8fafc', lineHeight: 1.2, marginBottom: 6 }}>
                        {r.place_identity_lock?.name}
                    </div>
                    {r.store_name_from_image && r.store_name_from_image !== 'Unknown' && (
                        <div style={{ fontSize: '0.78rem', color: '#60a5fa', marginBottom: 10 }}>OCR read from signboard: "{r.store_name_from_image}"</div>
                    )}
                    ...
                    <div style={{ display: 'flex', gap: '1.5rem' }}>
                        <div>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                                ...
                                <span style={{ fontWeight: 700, color: '#fbbf24' }}>{ra.average_rating}</span>
                            </div>
                        </div>
                        <div>
                            <div style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', marginBottom: 4 }}>Reviews</div>
                            <div style={{ fontWeight: 700, color: '#f8fafc' }}>{ra.total_reviews?.toLocaleString()}</div>
                        </div>
                        <div>
                            <div style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', marginBottom: 4 }}>Store Type</div>
                            <div style={{ fontWeight: 700, color: '#e2e8f0', fontSize: '0.85rem' }}>{r.store_type}</div>
                        </div>
                        <div>
                            <div style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', marginBottom: 4 }}>Images</div>
                            <div style={{ fontWeight: 700, color: '#38bdf8' }}>{va.total_images_analyzed}</div>
                        </div>
                    </div>
                </Panel>

                <Panel glowColor={scoreColor} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1rem' }}>
                    <AnimatedRing score={r.authenticity_score} size={150} label="Auth Score" />
                    <div style={{ padding: '0.3rem 1.1rem', borderRadius: 9999, fontSize: '0.78rem', fontWeight: 700, background: `${scoreColor}18`, border: `1px solid ${scoreColor}50`, color: scoreColor }}>
                        {r.authenticity_score >= 75 ? 'Authentic' : r.authenticity_score >= 50 ? 'Possibly Authentic' : 'Low Authenticity'}
                    </div>
                </Panel>
            </div>

            {/* ROW 2: Vision Stats + Score Breakdown */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                <Panel glowColor="#f97316">
                    <PanelHeader icon={Cpu} title="Layer 1 — YOLO Detection Stats" iconColor="#f97316" />
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
                        <StatBox label="Products Detected" value={va.total_products_detected} color="#f97316" />
                        <StatBox label="Unique Brands" value={va.unique_brands?.length ?? 0} color="#60a5fa" />
                        <StatBox label="Shelf Quality" value={`${va.shelf_quality_score}%`} color="#34d399" />
                    </div>
                    {r.layer1_yolo_summary?.map((img) => (
                        <div key={img.image_index} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem 0', borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: '0.79rem' }}>
                            <span style={{ color: 'rgba(255,255,255,0.5)' }}>Image {img.image_index}</span>
                            <div style={{ display: 'flex', gap: 12 }}>
                                <span style={{ color: '#f97316', fontWeight: 600 }}>{img.detections_count} total</span>
                                <span style={{ color: '#34d399', fontWeight: 600 }}>{img.fmcg_relevant_count} FMCG</span>
                            </div>
                        </div>
                    ))}
                </Panel>
                ...
                <Panel glowColor="#a78bfa">
                    <PanelHeader icon={TrendingUp} title="Authenticity Score Breakdown" iconColor="#a78bfa" />
            ...
            {/* ROW 3: Brand Distribution + Category Presence */}
            <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: '1.5rem' }}>
                <Panel>
                    <PanelHeader icon={Package} title="Layer 2 — Validated Brands (Qwen-VL)" iconColor="#a78bfa"
                        badge={{ label: `${va.unique_brands?.length ?? 0} validated`, bg: 'rgba(167,139,250,0.1)', border: 'rgba(167,139,250,0.3)', color: '#a78bfa' }} />
                    {va.unique_brands?.length > 0 ? (
                        <div>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginBottom: '1.25rem' }}>
                                {va.unique_brands.map((brand) => {
                                    const dist = va.brand_distribution?.[brand];
                                    const src = dist?.source || "";
                                    const srcColor = src.includes("YOLO") && src.includes("OCR") ? '#34d399'
                                        : src.includes("YOLO") ? '#f97316' : '#38bdf8';
                                    return (
                                        <span key={brand} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '0.25rem 0.7rem', borderRadius: 9999, fontSize: '0.78rem', fontWeight: 600, background: `${srcColor}12`, border: `1px solid ${srcColor}35`, color: srcColor }}>
                                            {brand}
                                            {src && <span style={{ fontSize: '0.6rem', opacity: 0.7 }}>· {src}</span>}
                                        </span>
                                    );
                                })}
                            </div>
                            <div style={{ display: 'flex', gap: 12, fontSize: '0.68rem', color: 'rgba(255,255,255,0.3)', flexWrap: 'wrap' }}>
                                <span><span style={{ color: '#f97316' }}>●</span> YOLO only</span>
                                <span><span style={{ color: '#38bdf8' }}>●</span> OCR only</span>
                                <span><span style={{ color: '#34d399' }}>●</span> YOLO+OCR</span>
                            </div>
                        </div>
                    ) : (
                        <div style={{ color: 'rgba(255,255,255,0.3)', textAlign: 'center', padding: '1.5rem', fontSize: '0.875rem' }}>No brands validated — images may be exterior-only.</div>
                    )}
                    {va.shelf_dominance && va.shelf_dominance !== 'Unknown' && (
                        <div style={{ marginTop: '1rem', padding: '0.65rem 0.9rem', borderRadius: '0.6rem', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', fontSize: '0.79rem', color: 'rgba(255,255,255,0.6)' }}>
                            <span style={{ color: 'rgba(255,255,255,0.35)', fontWeight: 700, textTransform: 'uppercase', fontSize: '0.65rem' }}>Shelf Dominance: </span>
                            {va.shelf_dominance}
                        </div>
                    )}
                </Panel>
            ...
            {/* ROW 4: Layer 3 OCR + Review Sentiment */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                <Panel>
                    <PanelHeader icon={ScanLine} title="Layer 3 — OCR Extracted Text" iconColor="#38bdf8" />
                    {r.layer3_ocr_summary?.some((img) => img.texts_extracted > 0) ? (
                        r.layer3_ocr_summary.map((img) =>
                            img.texts_extracted > 0 ? (
                                <div key={img.image_index} style={{ marginBottom: '1rem' }}>
                                    <div style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', marginBottom: 6 }}>Image {img.image_index}</div>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                                        {img.texts.slice(0, 6).map((t, i) => (
                                            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.35rem 0.65rem', borderRadius: '0.4rem', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', fontSize: '0.78rem' }}>
                                                <span style={{ color: '#f8fafc', fontWeight: 500 }}>{t.text}</span>
                                                <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.68rem' }}>{Math.round((t.confidence || 0) * 100)}%</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div style={{ color: 'rgba(255,255,255,0.3)', textAlign: 'center', padding: '1.5rem', fontSize: '0.875rem' }}>No readable text extracted from images.</div>
                    )}
                </Panel>
            ...
            {r.ai_insights?.length > 0 && (
                <Panel glowColor="#a78bfa">
                    <PanelHeader icon={Sparkles} title="Qwen-VL AI Insights" iconColor="#a78bfa" />
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                        {r.ai_insights.map((insight, i) => (
                            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '0.7rem 0.9rem', borderRadius: '0.6rem', background: 'rgba(167,139,250,0.07)', border: '1px solid rgba(167,139,250,0.2)' }}>
                                <span style={{ color: '#a78bfa', fontWeight: 700, fontSize: '0.75rem', minWidth: 20 }}>{i + 1}.</span>
                                <span style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.7)', lineHeight: 1.5 }}>{insight}</span>
                            </div>
                        ))}
                    </div>
                    <div style={{ marginTop: '1rem', fontSize: '0.68rem', color: 'rgba(255,255,255,0.25)', fontStyle: 'italic', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '0.75rem' }}>
                        Insights generated by Qwen2.5-VL-7B-Instruct from YOLO+OCR structured detection data only. No assumptions made.
                    </div>
                </Panel>
            )}
        </div>
